...
function FloatingCard({
  ...
  return (
    ...
    >
      <div className="rounded-2xl bg-white/90 backdrop-blur-sm border border-white/60 shadow-[0_12px_48px_rgba(124,58,237,0.1)] overflow-hidden">
        {children}
      </div>
    </motion.div>
  );
...
export default function Hero({ ready }: { ready: boolean }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-[1400px] mx-auto h-[400px] sm:h-[500px] lg:h-[550px] mt-6">

        {/* === LEFT COLUMN === */}

        {/* Score Card */}
        <FloatingCard className="hidden lg:block top-[5%] left-[2%] w-52" rotate={-3} delay={0.9} fromX={-40} ready={ready}>
          <div className="p-5">
        ...
        {/* Lending Pool Card */}
        <FloatingCard className="hidden lg:block top-[52%] left-[1%] w-56" rotate={2} delay={1.1} fromX={-40} ready={ready}>
          <div className="p-5">
        ...
        <FloatingCard className="hidden md:block bottom-[0%] left-1/2 -translate-x-1/2 w-[92%] max-w-2xl" rotate={0} delay={1.0} fromY={50} ready={ready}>
          <div className="p-6">
            {/* Header */}
            ...
            <div className="space-y-2.5">
              {[
                ...
                <motion.div
                  key={i}
                  className="flex items-center justify-between p-3 rounded-xl bg-white/40 border border-white/30"
                  initial={{ opacity: 0, x: -15 }}
                  animate={ready ? { opacity: 1, x: 0 } : { opacity: 0, x: -15 }}
                  transition={{ duration: 0.4, delay: 1.6 + i * 0.1, ease }}
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center text-sm" style={{ backgroundColor: item.color + '15', color: item.color }}>
                      {item.icon}
                    </div>
                    <div>
                      <div className="font-sans text-xs font-medium" style={{ color: colors.text }}>{item.type}</div>
                      <div className="font-mono text-[9px]" style={{ color: colors.textMuted }}>{item.detail}</div>
                    </div>
                  </div>
                  <div className="text-right">
        ...
        {/* Loan Card */}
        <FloatingCard className="hidden lg:block top-[2%] right-[2%] w-52" rotate={3} delay={1.0} fromX={40} ready={ready}>
          <div className="p-5">
            <div className="flex items-center justify-between mb-3">
        ...
        {/* Yield Chart Card */}
        <FloatingCard className="hidden lg:block top-[48%] right-[1%] w-56" rotate={-2} delay={1.2} fromX={40} ready={ready}>
          <div className="p-5">
            <div className="flex items-center justify-between mb-3">
